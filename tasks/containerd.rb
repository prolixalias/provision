...
def install_ssh_components(distro, version, container)
  ...
  when %r{debian}, %r{ubuntu}, %r{cumulus}
    ...
    run_local_command("nerdctl exec #{container} rm -f /etc/apt/sources.list.d/ubuntu-esm-infra-trusty.list")
    run_local_command("nerdctl exec #{container} apt-get update")
    run_local_command("nerdctl exec #{container} apt-get install -y openssh-server openssh-client")
  when %r{fedora}
  ...
  when %r{centos}, %r{^el-}, %r{eos}, %r{oracle}, %r{ol}, %r{redhat}, %r{scientific}, %r{amzn}, %r{rocky}, %r{almalinux}
    if version == '6'
      ...
      run_local_command("nerdctl exec #{container} bash -exc \"rm -f /var/lib/rpm/__db*; "\
        'db_verify /var/lib/rpm/Packages; '\
        'rpm --rebuilddb; '\
        'yum clean all; '\
        'yum install -y sudo openssh-server openssh-clients"')
    else
      run_local_command("nerdctl exec #{container} yum install -y sudo openssh-server openssh-clients")
    end
    ssh_folder = run_local_command("nerdctl exec #{container} ls /etc/ssh/")
    run_local_command("nerdctl exec #{container} ssh-keygen -t rsa -f /etc/ssh/ssh_host_rsa_key -N \"\"") unless ssh_folder.include?('ssh_host_rsa_key')
    run_local_command("nerdctl exec #{container} ssh-keygen -t dsa -f /etc/ssh/ssh_host_dsa_key -N \"\"") unless ssh_folder.include?('ssh_host_dsa_key')
  when %r{opensuse}, %r{sles}
    run_local_command("nerdctl exec #{container} zypper -n in openssh")
    run_local_command("nerdctl exec #{container} ssh-keygen -t rsa -f /etc/ssh/ssh_host_rsa_key")
    run_local_command("nerdctl exec #{container} ssh-keygen -t dsa -f /etc/ssh/ssh_host_dsa_key")
    run_local_command("nerdctl exec #{container} sed -ri \"s/^#?UsePAM .*/UsePAM no/\" /etc/ssh/sshd_config")
  when %r{archlinux}
    run_local_command("nerdctl exec #{container} pacman --noconfirm -Sy archlinux-keyring")
    run_local_command("nerdctl exec #{container} pacman --noconfirm -Syu")
    run_local_command("nerdctl exec #{container} pacman -S --noconfirm openssh")
    run_local_command("nerdctl exec #{container} ssh-keygen -A")
    run_local_command("nerdctl exec #{container} sed -ri \"s/^#?UsePAM .*/UsePAM no/\" /etc/ssh/sshd_config")
    run_local_command("nerdctl exec #{container} systemctl enable sshd")
  else
    raise "distribution #{distro} not yet supported on nerdctl"
  end

  # Make sshd directory, set root password
  run_local_command("nerdctl exec #{container} mkdir -p /var/run/sshd")
  run_local_command("nerdctl exec #{container} bash -c \"echo root:root | /usr/sbin/chpasswd\"")
end

def fix_ssh(distro, version, container)
  run_local_command("nerdctl exec #{container} sed -ri \"s/^#?PermitRootLogin .*/PermitRootLogin yes/\" /etc/ssh/sshd_config")
  run_local_command("nerdctl exec #{container} sed -ri \"s/^#?PasswordAuthentication .*/PasswordAuthentication yes/\" /etc/ssh/sshd_config")
  run_local_command("nerdctl exec #{container} sed -ri \"s/^#?UseDNS .*/UseDNS no/\" /etc/ssh/sshd_config")
  run_local_command("nerdctl exec #{container} sed -e \"/HostKey.*ssh_host_e.*_key/ s/^#*/#/\" -ri /etc/ssh/sshd_config")
  case distro
  when %r{debian}, %r{ubuntu}
    run_local_command("nerdctl exec #{container} service ssh restart")
  when %r{centos}, %r{^el-}, %r{eos}, %r{fedora}, %r{ol}, %r{redhat}, %r{scientific}, %r{amzn}, %r{rocky}, %r{almalinux}
    ...
    if distro =~ %r{redhat|centos} && version =~ %r{^7}
      run_local_command("nerdctl exec #{container} sed 's@session\s*required\s*pam_loginuid.so@session optional pam_loginuid.so@g' -i /etc/pam.d/sshd")
    end

    if !%r{^(7|8|2)}.match?(version)
      run_local_command("nerdctl exec #{container} service sshd restart")
    else
      run_local_command("nerdctl exec #{container} /usr/sbin/sshd")
    end
  else
...
def get_image_os_release_facts(image)
  os_release_facts = {}
  begin
    os_release = run_local_command("nerdctl run --rm #{image} cat /etc/os-release")
...
def provision(image, inventory_location, vars)
  ...
  creation_command = "nerdctl run -d -it --privileged #{deb_family_systemd_volume} --tmpfs /tmp:exec -p #{front_facing_port}:22 --name #{full_container_name} "
...
def tear_down(node_name, inventory_location)
  ...
  remove_nerdctl = "nerdctl rm -f #{node_facts['container_name']}"
  run_local_command(remove_nerdctl)
...
begin
  ...
  puts({ _error: { kind: 'provision/containerd_failure', msg: e.message, backtrace: e.backtrace } }.to_json)
  exit 1
end
